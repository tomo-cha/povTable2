...
file.write('const int NUMPIXELS = ' + str(PIXELS*NUMTAPES) +
           ';\n')
file.write('const int Div = ' + str(Div) + ';\n' + '\n')


file.write('uint32_t vpic [Div][NUMPIXELS] = {' + '\n')

# 画像ファイルを読み込む(png,jpg,bmpなどが使用可能)
pic1 = "src/bangaku/ban.png"  # 1枚目の画像
pic2 = "src/bangaku/gaku.png"  # 2枚目の画像
pic3 = "src/bangaku/haku.png"  # 3枚目の画像
pic4 = "src/bangaku/ran.png"  # 4枚目の画像
pic5 = "src/bangaku/kai.png"  # 5枚目の画像
...
def polarConv(pic, n):
    ...
    for j in range(0, Div):
        ...
        for i in range(0, hC+1):
            ...
            if (n % 2 == 1):
                l[j][i+(n-1) *
                     PIXELS] = '0x{:02X}{:02X}{:02X}'.format(rP, gP, bP)
            else:
